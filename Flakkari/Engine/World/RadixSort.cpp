...
void radix_sort_u64_indices_scratch(vector<uint64_t>& keys, vector<int>& indices, vector<uint64_t>& tmpKeys, vector<int>& tmpIndices) {
    size_t n = keys.size();
    if (n == 0) return;

    // Ensure scratch buffers are sized
    if (tmpKeys.size() < n) tmpKeys.resize(n);
    if (tmpIndices.size() < n) tmpIndices.resize(n);
...
void radix_sort_u64_indices_b16_scratch(vector<uint64_t>& keys, vector<int>& indices, vector<uint64_t>& tmpKeys, vector<int>& tmpIndices) {
    size_t n = keys.size();
    if (n == 0) return;
    ...
    for (int pass = 0; pass < 4; ++pass) {
        ...
        if (count.size() < 65536) count.resize(65536);
